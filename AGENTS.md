...
### Backend
- `_test.go` にテーブル駆動で `TestXxx` を作成
- 実行: `make test` または `go test -cover ./...`
- 副作用はモック/インメモリを使用

### Frontend
- クリティカルなUI/ロジックは追加テストを推奨（Vitest/RTL）
- E2Eは Playwright を想定し `frontend/tests` 配下に配置
...
- **データベース**: RDS PostgreSQL（セッション管理、プレイヤー回答、メッセージ保存）
